...
Only reviewed implementations are called by the main program. `FB_TimeService`, the `FB_IOManager` safety core, the linear/cyclic automatic `FB_Selector` core, and the deterministic `FB_Blower` core are implemented under `07_Implementation/Function_Blocks`; physical channel configuration, Selector service jog, and Dosing follow.
