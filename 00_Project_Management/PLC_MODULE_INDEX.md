...
Only reviewed implementations are called by the main program. `FB_TimeService`, the `FB_IOManager` safety core, and the linear/cyclic automatic `FB_Selector` core are implemented under `07_Implementation/Function_Blocks`; physical channel configuration, Selector service jog, Blower, and Dosing follow.
